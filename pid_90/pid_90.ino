...
// Variable para guardar la última lectura válida de sensores (no 111)
...
int readSensors(int &Ls, int &Cs, int &Rs) {
  ...
  int data = (Ls << 2) | (Cs << 1) | Rs;

  if (data != 0b000) {  // Guardar sólo si no perdió la línea
    lastValidSensors = data;
  }
...
int calculateError(int sensorsData, int previousErrorLocal) {
  int errorLocal;
  switch (sensorsData) {
    case 0b100: errorLocal = 4; break;  // Negro es 1, blanco es 0.
    case 0b110: errorLocal = 2; break;
    case 0b010: errorLocal = 0; break;
    case 0b011: errorLocal = -2; break;
    case 0b001: errorLocal = -4; break;

    case 0b000:
      // Si perdió la línea, decide giro según última lectura válida
      if (lastValidSensors == 0b100 || lastValidSensors == 0b110) {
        errorLocal = 12;  // curva a la izquierda (girar fuerte a la izquierda)
      } else if (lastValidSensors == 0b001 || lastValidSensors == 0b011) {
        errorLocal = -12; // curva a la derecha (girar fuerte a la derecha)
      } else {
        errorLocal = previousErrorLocal; // si no sabe, mantiene el error anterior
      }
      break;
